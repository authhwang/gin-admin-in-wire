...
import (
	...
	"github.com/authhwang/gin-admin/internal/app/middleware"
	"github.com/authhwang/gin-admin/internal/app/routers/api"
	"github.com/authhwang/gin-admin/pkg/logger"

	"github.com/authhwang/gin-admin/internal/app/routers/api/ctl"
	"github.com/authhwang/gin-admin/pkg/auth"
...
func InitWeb(a auth.Auther, e *casbin.Enforcer, cDemo *ctl.Demo, cLogin *ctl.Login, cMenu *ctl.Menu, cRole *ctl.Role, cUser *ctl.User) *gin.Engine {
	cfg := config.Global()
	gin.SetMode(cfg.RunMode)
	...
	// 注册/api路由
	api.RegisterRouter(app, a, e, cDemo, cLogin, cMenu, cRole, cUser)
...
func InitHTTPServer(ctx context.Context, a auth.Auther, e *casbin.Enforcer, cDemo *ctl.Demo, cLogin *ctl.Login, cMenu *ctl.Menu, cRole *ctl.Role, cUser *ctl.User) func() {
	cfg := config.Global().HTTP
	addr := fmt.Sprintf("%s:%d", cfg.Host, cfg.Port)
	srv := &http.Server{
		Addr:         addr,
		Handler:      InitWeb(a, e, cDemo, cLogin, cMenu, cRole, cUser),
		ReadTimeout:  5 * time.Second,
		WriteTimeout: 10 * time.Second,
		IdleTimeout:  15 * time.Second,
	}
